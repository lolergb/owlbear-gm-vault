...
/** Whether a URL points to a supported OneDrive Personal host. */
export function isOneDriveUrl(value) {
  const safeUrl = sanitizeHttpUrl(value);
...
/**
 * Only OneDrive Personal URLs generated by the official Embed action are
 * allowed to retain Microsoft origin privileges inside a document frame.
 */
export function sanitizeOneDriveEmbedUrl(value) {
  const safeUrl = sanitizeHttpUrl(value);
  if (!safeUrl) return '';
  ...
  try {
    ...
    if (
      hostname === '1drv.ms' &&
      parsed.searchParams.get('em') === '2' &&
      /^\/[a-z]\/[a-z]\/[a-f0-9]{16}\/[A-Za-z0-9_-]+\/?$/i.test(parsed.pathname)
    ) {
      return parsed.href;
    }
...
/**
 * Accept a normal HTTP(S) page URL or extract the safe src from a OneDrive
 * iframe copied from OneDrive's Embed panel. No other iframe markup is kept.
 */
export function normalizePageUrlInput(value) {
  const raw = String(value ?? '').trim();
  if (!raw) return '';
  ...
  if (/^<iframe\b/i.test(raw)) {
    try {
      ...
      const frames = parsedDocument.querySelectorAll('iframe');
      if (frames.length !== 1) return '';
      return sanitizeOneDriveEmbedUrl(frames[0].getAttribute('src'));
    } catch {
      return '';
    }
  }
...
function sanitizeElementAttributes(
  ...
  if (element.tagName === 'IFRAME') {
    ...
    const trustedEmbed = sanitizeVideoEmbedUrl(frameUrl) ||
      sanitizeGoogleEmbedUrl(frameUrl) ||
      sanitizeOneDriveEmbedUrl(frameUrl);
    const sandboxTokens = [
      'allow-scripts',
      'allow-forms',
      'allow-popups',
